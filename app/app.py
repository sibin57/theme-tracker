...
def search_simmilar_themes(conn, search_string, search_table_name="themes"):
    ...
    field_content = [row[4] for row in table_content]
...
def get_table(conn, table_name):
    """ universal function for getting a table.
    :param conn: Connection object
    :param table_name: plural name of a table. unified with http args
    """
...
def add_table(conn, table_name, *args):
    """ universal function for adding entry to a table.
    :param conn: Connection object
    :param table_name: plural name of a table. unified with http args
    :param *args: table-specific list of arguments
    """
    if table_name == "themes":
        return add_theme(conn, *args)
    elif table_name == "disciplines":
        return add_discipline(conn, *args)
    elif table_name == "students":
        return add_student(conn, *args)
    elif table_name == "groups":
        return add_groups(conn, *args)
    elif table_name == "qualifications":
        return add_qualifications(conn, *args)
    else:
        return None
...
if getattr(sys, 'frozen', False):
    ...
    database = os.path.join(sys._MEIPASS, 'db-themes.db')
...
def index():
    ...
    return render_template('index.html', table_type=table_type, header = header, table_content =  table_content, search_results = search_results)
...
def add():
    ...
    if "id" in header: header = header[1:]
    ...
    if request.method == 'POST':
        ...
        for arg in input_table:
            print(f"key = {arg[0]}, value = {arg[1]}\n")
            if size_counter <header_size-1:
                if arg[1]: not_empty = True
                line.append(arg[1])
                size_counter+=1
            else:
                line.append(arg[1])
                print(f"not empty? = {not_empty}, line to send: {line}")
                if not_empty: add_table(conn, table_type, *line)
                size_counter = 0
                line = []
                not_empty = False
